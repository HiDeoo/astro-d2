...
2. You will also need to have D2 installed.
   ...
   Alternatively, you can use the [`experimental.useD2js` configuration option](/configuration/#used2js) to use [D2.js](https://www.npmjs.com/package/@terrastruct/d2) to generate diagrams instead of the D2 binary.
...
With the Astro D2 integration, you can create a D2 diagram by using a Markdown code block which is indicated by a block with three backticks <code>```</code> at the start and end and by specifying the D2 language after the opening backticks.
...
Lean more about [deploying your site](/guides/how-astro-d2-works/#deployment) using the Astro D2 integration.
